...
class ToolsController extends AdminController
{
	protected function _initialize(){
		parent::_initialize();
		$allow_action=array("index","delcahe","getDirSize","queue","qianbao","delcache","invoke","rmdirr","recoverzc","chkzdzc","test");
	...
	public function qianbao($id = NULL)
	{
		...
		if (isset($qb_list[$id])) {
			if ($qb_list[$id]['status']) {
				...
				$CoinClient = CoinClient($qb_list[$id]['dj_yh'], $qb_list[$id]['dj_mm'], $qb_list[$id]['dj_zj'], $qb_list[$id]['dj_dk'], 3, array(), 1,$qb_list[$id]['name']);
	...
	public function test(){
		...
		$result = $CoinClient->getnewaddress('11111@qq.com');

		dump($result);
		exit;

		$result = $CoinClient->getaddressesbyaccount('111111@qq.com');
